...
const orderTotal = products.reduce((acc, cv)) => {
    if(cv.color.includes('blue')) {
      console.log(cv)
      return acc + cv.price
    }
    return acc;
...
const updatedUserInfo = {...userInfo, topics: [userInfo.topics[userInfo.topics.length - 1]]}
...
const person = {
  name: "Michelle",
  age: 38,
  jobs: ["chef","driver", "nanny"],
  birthday:  function(){ return this.age + 1},
  favorites: {
            color: "green",
            number: 5,
            book: "The Isis Papers"
          },
  kids:[{
        name: "Sarah",
        age: 7
